...
public class SampleHandler extends AbstractHandler {
    ...
    public Object execute(ExecutionEvent event) throws ExecutionException {
        ...
        String resultsBaseDir = "C:/Users/X1502/Adriana/LAB334/MO-ILP/SSBSE_ASE_2026/Nuevo_revision_ASE_SSBSE2026/experimentos_nuevos/output-2026";

        // 🆕 Ahora se admite una LISTA de algoritmos. Si solo pones uno, el comportamiento es
        // exactamente el mismo que antes. Si pones varios, se ejecuta primero con el primero de
        // la lista y luego con el siguiente (una pasada completa -log, refactor y CSVs- por
        // cada combinación proyecto x algoritmo).
        List<String> targetAlgos = Arrays.asList("EpsilonConstraintAlgorithm","HybridMethodAlgorithm");
        // Ejemplo para lanzar los dos: Arrays.asList("EpsilonConstraintAlgorithm", "HybridMethodAlgorithm")

        List<String> userPriority = Arrays.asList("loc", "extractions", "cc");
        String targetClass = "";          // Opcional: Nombre de clase (ej. "JSON.java" o "")

        // 🆕 projectSourceDir ahora admite DOS cosas:
        //   1) Un proyecto concreto (carpeta o .zip) -> comportamiento IDÉNTICO al actual.
        //   2) Una carpeta "contenedora" con varios proyectos (carpetas y/o .zip) dentro de ella
        //      -> se procesan uno a uno, y la carpeta de salida "<proyecto>_refactored_<algoritmo>"
        //      de cada uno se genera dentro de esa misma carpeta contenedora (igual que se genera
        //      hoy junto al proyecto individual).
        String projectSourceDir = "C:/Users/X1502/Downloads/Knowage-Server";
        String targetSubmodule = "knowage-core";
        ...
        Job refactoringJob = new Job("Procesando Refactorizaciones MO-ILP") {
            ...
            protected IStatus run(IProgressMonitor monitor) {
                ...
                File sourceRoot = new File(projectSourceDir);
                List<File> projectsToProcess = resolveProjectsToProcess(sourceRoot, targetSubmodule);
 
                if (projectsToProcess.isEmpty()) {
                    System.err.println("⚠️ No se encontró ningún proyecto que procesar en: " + projectSourceDir);
                    return Status.CANCEL_STATUS;
                }
 
                if (projectsToProcess.size() > 1) {
                    System.out.println("📚 Modo batch: se han detectado " + projectsToProcess.size()
                            + " proyecto(s) dentro de '" + sourceRoot.getAbsolutePath() + "'. Se procesarán uno por uno.");
                } else {
                    System.out.println("📄 Modo proyecto único: '" + projectsToProcess.get(0).getAbsolutePath() + "'.");
                }
                if (targetAlgos.size() > 1) {
                    System.out.println("🔁 Se ejecutarán " + targetAlgos.size() + " algoritmos por proyecto: " + targetAlgos);
                }
 
                boolean anyFailed = false;
 
                // 🆕 Bucle proyecto x algoritmo. Cada combinación es completamente independiente:
                // su propio log, su propia carpeta "_refactored_<algoritmo>" y sus propios CSVs.
                // Si una combinación falla, se avisa pero se continúa con el resto del batch
                // (un proyecto roto no debe abortar una tanda de experimentos nocturna completa).
                for (File project : projectsToProcess) {
                    for (String algo : targetAlgos) {
                        IStatus status;
                        try {
                            status = processSingleProject(project, algo, resultsBaseDir, userPriority, targetClass, targetSubmodule);
                        } catch (Exception e) {
                            System.err.println("❌ Fallo inesperado procesando '" + project.getName() + "' con " + algo + ": " + e.getMessage());
                            e.printStackTrace();
                ...
                return anyFailed ? Status.CANCEL_STATUS : Status.OK_STATUS;
            }
        };
    ...
    // =================================================================
    // 🆕 DETECCIÓN: ¿proyecto único o carpeta contenedora de varios proyectos?
    // =================================================================
    /**
     * Si sourceRoot es un .zip, o una carpeta que ya parece ser la raíz de un proyecto (tiene
     * pom.xml, build.xml o src en su primer nivel), se trata como proyecto único, exactamente
     * igual que hasta ahora.
     *
     * 🔧 FIX (regresión reportada): antes de asumir que, en caso contrario, sourceRoot es una
     * carpeta "contenedora" de VARIOS proyectos, contamos cuántos hijos directos parecen
     * proyectos por sí mismos (carpetas con pom.xml/build.xml/src, o ficheros .zip). Si hay 0 ó 1,
     * NO es realmente una carpeta contenedora: es un único proyecto cuya raíz real puede estar un
     * nivel más abajo (el típico "wrapper" de un ZIP de GitHub descomprimido a mano, el mismo caso
     * que ya gestiona ProjectUtils.findUnzippedProjectRoot para el flujo de .zip), o directamente
     * una carpeta sin marcadores estándar reconocibles. En ambos casos se sigue tratando como
     * proyecto ÚNICO -> la copia de trabajo se sigue haciendo FUERA de sourceRoot (como carpeta
     * hermana), nunca dentro de él. Esto es justo el comportamiento que había antes de introducir
     * el modo batch y que no debía romperse: solo se entra en modo batch de verdad cuando hay 2 o
     * más proyectos candidatos directos dentro de sourceRoot.
     *
     * 🔧 FIX (segunda regresión reportada): un proyecto Maven/Gradle MULTI-MÓDULO cuyos módulos
     * están todos como hermanos directos de sourceRoot (p.ej. Knowage-Server: "knowage",
     * "knowage-core", "knowage-vertical"... todos con su propio pom.xml al mismo nivel) también
     * tiene 2+ "candidatos", así que sin más comprobación se activaría el modo batch y se
     * refactorizaría CADA módulo como si fuera un proyecto independiente, ignorando por completo
     * "targetSubmodule". Si se ha especificado targetSubmodule, esto NUNCA puede ser modo batch:
     * se trata siempre como proyecto único, y es la propia lógica de búsqueda de submódulo (más
     * abajo, en processSingleProject) la que ya sabe encontrarlo dentro del árbol completo, tanto
     * si está justo un nivel por debajo como más profundo.
     */
    private static List<File> resolveProjectsToProcess(File sourceRoot, String targetSubmodule) {
        ...
        if (sourceRoot.isFile() && sourceRoot.getName().toLowerCase().endsWith(".zip")) {
        ...
        if (sourceRoot.isDirectory()) {
            ...
            if (targetSubmodule != null && !targetSubmodule.trim().isEmpty()) {
                // Se pidió un submódulo concreto -> proyecto único siempre, nunca batch.
                result.add(sourceRoot);
                return result;
            }
 
            List<File> candidateChildren = new ArrayList<>();
            File[] children = sourceRoot.listFiles();
            if (children != null) {
                for (File child : children) {
                    if (child.isFile() && child.getName().toLowerCase().endsWith(".zip")) {
                        candidateChildren.add(child);
                    } else if (child.isDirectory() && looksLikeProjectRoot(child)) {
                        candidateChildren.add(child);
                    }
                }
            }
 
            if (candidateChildren.size() >= 2) {
                // Contenedora real de varios proyectos -> modo batch, tal cual se pidió.
                // 🔧 candidateChildren es un List<File>, no un File[]: se ordena con
                // List.sort(...), no con Arrays.sort(...) (que es para arrays).
                candidateChildren.sort((a, b) -> a.getName().compareToIgnoreCase(b.getName()));
                result.addAll(candidateChildren);
                return result;
            }
 
            // 0 ó 1 candidato dentro: se trata como proyecto único (aunque su raíz real esté un
            // nivel más abajo; eso lo resuelve processSingleProject vía
            // ProjectUtils.findUnzippedProjectRoot sobre la copia ya hecha fuera del original).
            result.add(sourceRoot);
            return result;
        }
    ...
    private static boolean looksLikeProjectRoot(File dir) {
        return new File(dir, "pom.xml").exists()
                || new File(dir, "build.xml").exists()
                || new File(dir, "src").exists();
    }
 
    // =================================================================
    // 🆕 PURGA SEGURA DE PROYECTOS DE ECLIPSE (evita borrar la salida de OTRA ejecución)
    // =================================================================
    /**
     * Los proyectos de Eclipse que registra ASTModifier se identifican solo por NOMBRE
     * (p.ej. el nombre de la carpeta interna de un .zip descomprimido, como
     * "fastjson-93d8c01e..."), y ese nombre es el MISMO sin importar para qué algoritmo se
     * descomprimió el zip. Si simplemente se borra "el proyecto con ese nombre" con
     * deleteContent=true, se puede acabar borrando en disco la carpeta de OTRO algoritmo que
     * ya había terminado (porque su proyecto se quedó registrado en Eclipse y nunca se
     * desregistró al terminar esa ejecución).
     *
     * Esta purga solo borra el contenido en disco si el proyecto YA registrado apunta
     * exactamente a la carpeta que se va a reutilizar en ESTA ejecución (p.ej. restos de un
     * intento anterior fallido del mismo proyecto+algoritmo). Si apunta a otro sitio, se
     * desregistra la referencia de Eclipse sin tocar esos ficheros.
     */
    private static void purgeProjectReference(org.eclipse.core.resources.IProject project, File expectedLocation) {
        if (project == null || !project.exists()) {
            return;
        }
        try {
            org.eclipse.core.runtime.IPath currentLoc = project.getLocation();
            boolean sameLocation = currentLoc != null && expectedLocation != null
                    && currentLoc.toFile().getAbsolutePath().equals(expectedLocation.getAbsolutePath());
 
            project.delete(sameLocation, true, new org.eclipse.core.runtime.NullProgressMonitor());
 
            if (sameLocation) {
                System.out.println("🧹 Proyecto previo '" + project.getName() + "' purgado del workspace de Eclipse (contenido en disco incluido: misma ubicación).");
            } else {
                System.out.println("🧹 Proyecto previo '" + project.getName() + "' desregistrado de Eclipse (su contenido en disco pertenece a OTRA ejecución y se ha conservado).");
            }
        } catch (Exception e) {
            // Silenciamos igual que antes
        }
    }
 
    // =================================================================
    // Cuerpo original de run(), ahora parametrizado por (proyecto, algoritmo) para poder
    // llamarlo tanto en modo "proyecto único" como en el bucle "varios proyectos x varios
    // algoritmos" de arriba. La lógica interna es la misma de siempre; lo único añadido es
    // el paso 6b (CSV de tiempos de extracción).
    // =================================================================
    private static IStatus processSingleProject(File projectSourceFile, String targetAlgo, String resultsBaseDir,
            List<String> userPriority, String targetClass, String targetSubmodule) {
        ...
        // Nota: al usar projectSourceFile.getParentFile(), en modo batch esto cae automáticamente
        // dentro de la carpeta contenedora (porque el "proyecto" es uno de sus hijos directos),
        // que es justo el comportamiento pedido.
        ...
        // El Log se guarda dentro de la carpeta workingPath
        ...
        try {
            ...
            // 🔧 Purga segura: solo se borra el contenido en disco si "oldProject" apunta a la
            // carpeta que vamos a reutilizar en ESTA ejecución (workingPath); si apunta a otro
            // sitio, solo se desregistra de Eclipse.
            purgeProjectReference(oldProject, workingPath);
            ...
            // 🔧 FIX (regresión reportada): antes esto solo se aplicaba si originalName era un
            // .zip ("IGNORAR findUnzippedProjectRoot si estamos copiando una carpeta directa").
            // Con el nuevo resolveProjectsToProcess de arriba, un sourceRoot no-zip que en su
            // propio nivel no tenga pom.xml/build.xml/src (pero SÍ los tenga un único hijo
            // directo) ya no se malinterpreta como "carpeta contenedora": se trata como proyecto
            // único y se copia entero (workingPath) fuera del original. Para que en ese caso
            // projectRoot apunte a la raíz REAL (el hijo con pom.xml/build.xml/src) y no a la
            // carpeta envoltorio copiada, aplicamos el mismo findUnzippedProjectRoot que ya se
            // usaba para .zip también aquí. findUnzippedProjectRoot ya devuelve 'workingPath' sin
            // cambios si este ya es la raíz real, así que es seguro para el caso normal (carpeta
            // que ya era directamente el proyecto).
            File foundTargetDir = ProjectUtils.findUnzippedProjectRoot(workingPath);
            File projectRoot = (foundTargetDir != null) ? foundTargetDir : workingPath;
            String projectName = cleanName;
            String generalProjectName = cleanName;
 
            // --- BÚSQUEDA PROFUNDA Y RECURSIVA DEL SUBMÓDULO REAL ---
            if (targetSubmodule != null && !targetSubmodule.trim().isEmpty()) {
            ...
            // 🔧 Este es el punto crítico: el nombre de "projectRoot" (p.ej. la carpeta interna de
            // un .zip descomprimido, como "fastjson-93d8c01e...") es el MISMO para cualquier
            // algoritmo, porque viene del mismo .zip. Sin esta comprobación de ubicación, al
            // procesar el segundo algoritmo se borraría en disco la carpeta ya refactorizada del
            // primero (que se quedó registrada en Eclipse sin desregistrar). Ahora solo se borra
            // el contenido si "oldSubProject" apunta EXACTAMENTE a este projectRoot; si apunta a
            // la carpeta de otro algoritmo, solo se desregistra de Eclipse sin tocar esos ficheros.
            ...
            purgeProjectReference(oldSubProject, projectRoot);
            ...
            // =================================================================
            // 🆕 6b. EXPORTAR CSV DE TIEMPOS DE EXTRACCIÓN
            // Una fila por cada extracción realmente intentada (nombre, offsets aplicados tras el
            // ajuste semántico AST, tiempo de ejecución en ms, y si tuvo éxito o no). Se guarda en
            // la misma carpeta que el log y el CSV de rechazos.
            // =================================================================
            File extractionTimingCsvFile = new File(workingPath, cleanName + "_" + targetAlgo + "_extractions_timing_v2026-06.csv");
            try (java.io.PrintWriter csvWriter = new java.io.PrintWriter(new java.io.FileWriter(extractionTimingCsvFile))) {
                csvWriter.println("Clase;MetodoOriginal;NombreExtraccion;OffsetInicio;OffsetFin;TiempoEjecucionMs;Exitosa");
                for (Map.Entry<String, Map<String, List<ResultsProcessor.Extraction>>> classEntry : resultsMap.entrySet()) {
                    String className = classEntry.getKey();
                    for (Map.Entry<String, List<ResultsProcessor.Extraction>> methodEntry : classEntry.getValue().entrySet()) {
                        String methodName = methodEntry.getKey();
                        for (ResultsProcessor.Extraction ext : methodEntry.getValue()) {
                            // Las extracciones descartadas ANTES de intentarse (posición inválida/
                            // eliminada) nunca llegan a tener tiempo asignado (executionTimeMs sigue
                            // en -1): no tiene sentido registrar una fila con un tiempo inexistente.
                            if (ext.executionTimeMs < 0) {
                                continue;
                            }
                            try {
                                String extractedName = (ext.extractedMethodName != null) ? ext.extractedMethodName : "";
                                csvWriter.println(className + ";" + methodName + ";" + extractedName + ";"
                                        + ext.appliedStartOffset + ";" + ext.appliedEndOffset + ";"
                                        + ext.executionTimeMs + ";" + ext.succeeded);
                            } catch (Exception innerE) {
                                System.err.println("⚠️ Aviso: No se pudo escribir la fila de tiempos para " + methodName + ". Motivo: " + innerE.getMessage());
                            }
                        }
                    }
                }
                System.out.println("💾 Archivo CSV con tiempos de extracción exportado en:\n   " + extractionTimingCsvFile.getAbsolutePath());
            } catch (Exception e) {
                System.err.println("⚠️ Error crítico al abrir o guardar el CSV de tiempos de extracción: " + e.getMessage());
            }
